...
class _DriverActiveRideScreenState
  ...
  Future<void> _cancelRide() async {
    ...
    try {
      final response = await ref
          .read(apiClientProvider)
          .post('/rides/${widget.rideId}/cancel', data: {'reason': reason});

      developer.log(
        'CANCEL RIDE RESPONSE: ${response.data}',
        name: 'ActiveRide',
      );
      ...
      if (status != 'CANCELLED') {
        _showError('Could not cancel the ride. Please try again.');
        return;
      }
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text('This will notify the rider immediately.'),
